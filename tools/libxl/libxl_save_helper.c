...
static volatile sig_atomic_t usr_signalled;

static void usr_signal_handler(int num)
{
    /* XXX more detail - essentially this is used by the parent process to
     * indicate that an in-progress live migration save should transition into
     * the post-copy phase */
    usr_signalled = true;
}

static int usr_signal_received(void *data)
{
    return usr_signalled;
}

static void setup_signals(void (*sigterm_handler)(int),
                          void (*sigusr1_handler)(int))
{
    ...
    sigaddset(&spmask,SIGUSR1);
    r = sigprocmask(SIG_UNBLOCK,&spmask,0);
    if (r) fail(errno,"sigprocmask unblock SIGTERM/SIGUSR1 failed");
}
...
#define NEXTARG (++argv, assert(*argv), *argv)
    ...
    if (!strcmp(mode,"--save-domain")) {
        ...
        helper_save_callbacks.should_begin_postcopy = usr_signal_received;

        startup("save");
        setup_signals(save_signal_handler, usr_signal_handler);
