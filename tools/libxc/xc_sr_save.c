...
static const bool batch_includes_contents[] =
{
    ...
    [XC_SR_SAVE_BATCH_POSTCOPY_PAGE] = false
};
                       ...
                       xen_pfn_t *types)
{
    ...
    bmfns = malloc(nr_pfns * sizeof(*errors));
...
static int postcopy_domain_memory(struct xc_sr_context *ctx)
{
    ...
    complete = rec.data;
    ctx->save.callbacks->postcopy_results(complete->result,
                                          ctx->save.callbacks->data);

    if ( complete->result != POSTCOPY_COMPLETE_SUCCESS )
    {
        rc = -1;
        goto err;
    }
