...
struct restore_callbacks {
    ...
    union {
        struct {
            ...
            /* Called upon completion of the postcopy memory restore to
             * synchronize with the resumption of the guest and reap the result
             * of the resumption.
             *
             * Returns:
             *  < 0                                on failure to synchronize
             *  POSTCOPY_COMPLETE_SUCCESS          on resume success
             *  POSTCOPY_COMPLETE_FAILED_PRERESUME on failure to resume */
            int (*postcopy_synchronize)(void *data);
        };
